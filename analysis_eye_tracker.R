...
my.data<- 
  ...
  mutate(strength_conf2=(dwell_chosen-dwell_unchosen)/(dwell_chosen+dwell_unchosen))
...
my.data<-
  ...
  mutate(bin_strength = ntile(strength_conf, 7),bin_weight = ntile(weight_conf, 7),bin_strength2 = ntile(strength_conf2, 7))
...
ggplot(aes(y=zConf,x=bin_weight),data=c_a_per_weight)+geom_point()+facet_wrap(~participant,scales="free_y")+geom_line()+geom_errorbar(aes(ymin=ymin,ymax=ymax),width=0.1)

c_a_per_strength<-
  my.data %>% 
  group_by(participant,bin_strength2) %>% 
  summarise(ZConf=mean(zConf),
            sd_cor=sd(zConf),
            N=n(),
            se_cor= sd(zConf)/ sqrt(n()),
            ymin=mean(zConf)-sd(zConf)/ sqrt(n()),
            ymax=mean(zConf)+sd(zConf)/ sqrt(n()))
ggplot(aes(y=ZConf,x=bin_strength2),data=c_a_per_strength)+geom_point()+facet_wrap(~participant,scales="free_y")+geom_line()+geom_errorbar(aes(ymin=ymin,ymax=ymax),width=0.1)
